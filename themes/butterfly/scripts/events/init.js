...
hexo.extend.filter.register('before_generate', () => {
  ...
  if (hexo.locals.get) {
    ...
    if (data && data.butterfly) {
      logger.error(" '_config.butterfly.yml' is deprecated. Please use '_config._config.butterfly.yml' ")
      logger.error(" '_config.butterfly.yml' 已經棄用，請使用 '_config._config.butterfly.yml' ")
      process.exit(-1)
    }
  }
